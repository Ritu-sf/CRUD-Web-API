...
import com.salesken.network.entity.Device;
import com.salesken.network.entity.DeviceTypes;

@Service
public class SaleskenServiceImplementation implements NetworkService {

	private Map<String, Device> deviceList = new HashMap<>();

	private ConcurrentHashMap<String, ArrayList<Device>> connectedNodes = new ConcurrentHashMap<>();
	...
	public String connectDevice(String source, List<String> targetList) {
		if (source.equals("") || targetList.isEmpty()) {
			...
			for (int i = 0; i < targetList.size(); i++) {
				if(deviceList.containsKey(targetList.get(i)))
	...
	public String routeInfo(String s, String t) {
		...
		if (!deviceList.containsKey(s) || !deviceList.containsKey(t) || s == "" || t == " ") {
			return "Invalid Request";
		}
		boolean RepeaterFound = false;

		if (deviceList.get(s).getType().toString().equals("REPEATER")
				|| deviceList.get(t).getType().toString().equals("REPEATER"))
			RepeaterFound = true;
	...
	public void removeConnection(String deviceName) {
		...
		if (connectedNodes.containsKey(deviceName)) {
			connectedNodes.remove(deviceName);

			for (Map.Entry<String, ArrayList<Device>> entry : connectedNodes.entrySet()) {
				ArrayList<Device> listofdevices = entry.getValue();

				Iterator<Device> iter = listofdevices.iterator();

				while (iter.hasNext()) {
					String dev = iter.next().getName();
